...
export function Sheet({ open, onClose, title, children, hideHeader = false }: SheetProps) {
  ...
  return (
    <div
      className={`fixed inset-0 z-30 flex items-end justify-center bg-black/50 backdrop-blur-sm transition-opacity duration-surface
    ...
    >
      <div
        ...
        className={`w-full max-w-lg rounded-t-3xl border-t border-white/10 bg-ink-1/95 p-7 pb-safe
          ...
          transition-transform duration-surface ease-enter
        ...
        {!hideHeader && (
          <div className="mb-6 flex items-center justify-between">
            ...
            <button
              ...
              aria-label="ปิด"
              data-modal-close
              className="flex h-9 w-9 items-center justify-center rounded-full text-fg-dim transition-all duration-surface ease-enter hover:bg-white/5 hover:text-fg active:scale-90"
            >
